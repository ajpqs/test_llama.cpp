...
ggml_tensor * llama_model_gigachat35::graph::build_zero_centered_norm(
        ggml_tensor * input,
        ggml_tensor * weight,
        int           il) {
...
ggml_tensor * llama_model_gigachat35::graph::build_zero_centered_gated_norm(
        ggml_tensor * input,
        ggml_tensor * weight,
        ggml_tensor * gate_up,
...
ggml_tensor * llama_model_gigachat35::graph::build_layer_attn_linear(
    ...
    if (num_k_heads != num_v_heads) {
        ...
        // HF checkpoint stores V heads grouped by K head, Q/K need repeat-interleave before GDN
...
ggml_tensor * llama_model_gigachat35::graph::build_layer_attn_mla(
...
ggml_tensor * llama_model_gigachat35::graph_mtp::build_zero_centered_norm(
        ggml_tensor * input,
        ggml_tensor * weight,
        int           il) {
    GGML_ASSERT(weight != nullptr);
...
ggml_tensor * llama_model_gigachat35::graph_mtp::build_zero_centered_gated_norm(
        ggml_tensor * input,
        ggml_tensor * weight,
        ggml_tensor * gate_up,
        ggml_tensor * gate_down,
...
ggml_tensor * llama_model_gigachat35::graph_mtp::build_layer_attn_mla(
        llm_graph_input_attn_k * inp_attn,
        ggml_tensor *           cur,
        ggml_tensor *           inp_pos,
        float                   kq_scale,
